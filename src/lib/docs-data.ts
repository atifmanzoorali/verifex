...
export async function getDocsData(keyPrefix?: string): Promise<DocsSection[]> {
  ...
  const validateCurl = `curl -X POST https://verifex.app/api/v1/validate \\
  -H "X-API-Key: ${key}" \\
  -H "Content-Type: application/json" \\
  -d '{"email": "user@example.com"}'`;

  const [
    validateCurlHtml, validHtml, invalidHtml,
    keysListHtml,
    keysCreateBodyHtml, keysCreateResponseHtml,
    keysRevokeBodyHtml, keysRevokeResponseHtml,
    usageHtml,
    accountHtml,
  ] = await Promise.all([
    h(validateCurl, 'bash'), h(VALID_RESPONSE, 'json'), h(INVALID_RESPONSE, 'json'),
    h(KEYS_LIST_RESPONSE, 'json'),
    h(KEYS_CREATE_BODY, 'json'), h(KEYS_CREATE_RESPONSE, 'json'),
    h(KEYS_REVOKE_BODY, 'json'), h(KEYS_REVOKE_RESPONSE, 'json'),
    h(USAGE_RESPONSE, 'json'),
    h(ACCOUNT_RESPONSE, 'json'),
  ]);
      ...
      endpoints: [
        {
          ...
          examples: [
            { label: 'Request', lang: 'bash', raw: validateCurl, html: validateCurlHtml },
      ...
      endpoints: [
        {
          ...
          examples: [
            { label: 'Response', lang: 'json', raw: KEYS_LIST_RESPONSE, html: keysListHtml },
          ],
        },
        {
          method: 'POST',
          ...
          auth: 'Supabase session cookie (browser only).',
          examples: [
            { label: 'Request body', lang: 'json', raw: KEYS_CREATE_BODY, html: keysCreateBodyHtml },
            { label: 'Response', lang: 'json', raw: KEYS_CREATE_RESPONSE, html: keysCreateResponseHtml },
          ],
        },
        {
          method: 'DELETE',
          ...
          auth: 'Supabase session cookie (browser only).',
          examples: [
            { label: 'Request body', lang: 'json', raw: KEYS_REVOKE_BODY, html: keysRevokeBodyHtml },
            { label: 'Response', lang: 'json', raw: KEYS_REVOKE_RESPONSE, html: keysRevokeResponseHtml },
          ],
        },
      ],
    },
      ...
      endpoints: [
        {
          ...
          auth: 'Supabase session cookie (browser only).',
          examples: [
            { label: 'Response', lang: 'json', raw: USAGE_RESPONSE, html: usageHtml },
          ],
        },
      ],
    },
      ...
      endpoints: [
        {
          ...
          auth: 'Supabase session cookie (browser only).',
          examples: [
            { label: 'Response', lang: 'json', raw: ACCOUNT_RESPONSE, html: accountHtml },
          ],
        },
      ],
    },
